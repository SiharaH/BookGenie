        ...
        try{
            if(token.type === "heading_open") {
                ...
                if(nextToken && nextToken.type === "inline") {
                    ...
                    i += 2; // Skip inline, and heading_close tokens
                }
                ...
                if(nextToken && nextToken.type === "inline" && nextToken.children) {
                    ...
                    if(textRuns.length > 0) {
                        paragraphs.push(new Paragraph({
                            children: textRuns,
                            spacing: {  before: inList ? 100: DOCX_STYLES.spacing.paragraphBefore, 
                                        after: inList ? 100 : DOCX_STYLES.spacing.paragraphAfter, line: 360
                                    },
                            alignment: AlignmentType.JUSTIFIED,
                        }));
                    }
                    i += 2; // Skip inline and paragraph_close tokens
                }
                ...
                if(nextToken && nextToken.type === "paragraph_open") {
                    ...
                    if(inlineToken && inlineToken.type === "inline" && inlineToken.children) {
                        ...
                        i += 4; // Skip paragraph_open, inline, and list_item_close, list_item_close tokens
                    }
                }
            }else if(token.type === "block_quote_open") {
                //find the content of the block quote
                const nextToken = tokens[i + 1];
                if(nextToken && nextToken.type === "paragraph_open") {
                    const inlineToken = tokens[i + 2];
...
const renderInlineTokens = (doc, tokens, options = {}) => {
    ...
    const baseOptions = {
        ...
        lineGap: Number.isFinite(options.lineGap) ? options.lineGap : 2,
    };
        ...
        if(book.coverImage && !book.coverImage.includes("pravatar")) {
            ...
            try{
                if(fs.existsSync(imagePath)) {
                    ...
                    doc.image(
                        imagePath,{fit: [pageWidth, pageHeight],align: "center", valign: "center"}
                    );
                    doc.addPage();
                }
    } catch (imgErr) {
