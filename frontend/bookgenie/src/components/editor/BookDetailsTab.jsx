...
const BookDetailsTab = ({
    ...
    const coverImageUrl = book.coverImage ? book.coverImage.startsWith('http') ? book.coverImage : 
    `${BASE_URL}/backend${book.coverImage}`.replace(/\\/g, '/') : null;
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Book Details</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField label="Title" name="title" value={book.title} onChange={onBookChange} placeholder="Enter book title" />
            <InputField label="Author" name="author" value={book.author} onChange={onBookChange} placeholder="Enter author name" />
            <div className="md:col-span-2">
                <InputField label="Subtitle" name="subtitle" value={book.subtitle || ''} onChange={onBookChange} placeholder="Enter book subtitle" />
            </div>
        </div>
      </div>
