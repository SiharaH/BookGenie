...
const CreateBookModal = ({ isOpen, onClose, onBookCreated }) => {
  ...
  const handleGenerateOutline = async () => {
    if(!bookTitle || !numChapters) {
      toast.error("Please enter a book title and number of chapters.");
      return;
    }
  ...
  const handleAddChapters = () => {
    setChapters([
      ...chapters, { title:`Chapter ${chapters.length + 1}`, description: ""},]);
  };

  const handleFinalizeBook = async () => {
    if(!bookTitle || chapters.length === 0) {
      toast.error("Book must have a title and at least one chapter.");
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to create book. Please try again.");
    } finally {
      setIsFinalizingBook(false);
    }
  };
      ...
      {step === 1 && (
        <div className='space-y-5'>
          <div className='flex items-center gap-2 mb-6'>
            <div className='flex items-center justify-center w-8 h-8 rounded-full bg-violet-100 text-purple-600 text-sm font-bold'>
              1
            </div>
          ...
          <SelectField
           ...
           <div className='flex justify-end pt-4'>
            <Button onClick={handleGenerateOutline} className='w-full flex items-center justify-center gap-2' isLoading={isGeneratingOutline} icon={Sparkles}>
        ...
        {step === 2 && (
          <div className='space-y-5'>
            ...
            <div className='flex items-center gap-2 mb-6'>
              ...
              <div className='flex items-center justify-center w-8 h-8 rounded-full bg-violet-100 text-violet-600 text-sm font-bold'>
                2
              </div>
            </div>
            ...
            <div ref={chapterContainerRef} className='space-y-3 max-h-96 overflow-y-auto pr-1'>
              {chapters.length === 0 ? (
                <div className='text-center py-12 px-4 bg-gray-50rounded-xl'>
                ...
                chapters.map((chapter, index) => (
                  <div key={index} className='group p-4 border border-gray-200 rounded-xl hover:border-gray-300 hover:shadow-sm transition-all bg-white'>
                    <div className='flex items-start gap-3 mb-3'>
                      ...
                      <input type="text" value={chapter.title} onChange={(e) => handleChapterChange(index, "title", e.target.value)}
                       placeholder='Chapter title' className='flex-1 text-base font-medium text-gray-900 bg-transparent border-none focus:outline-none focus:ring-0 p-0 ' />
                       <button onClick={() => handleDeleteChapter(index)} title='Delete Chapter' className='opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-50 transition-all'> 
                        <Trash2 className='w-4 h-4 text-red-500'/>
                        </button>
                    </div>
                    <textarea value={chapter.description} onChange={(e) => handleChapterChange(index, "description", e.target.value)} placeholder='Chapter description' rows={2} className='w-full pl-9 text-sm text-gray-600 bg-transparent border-none focus:outline-none focus:ring-0 resize-none placeholder-gray-400' />
                  </div>
                ))
              )}
            </div>
            ...
            <div className='flex items-center justify-between pt-4 border-t border-gray-100'>
              ...
              <div className='flex gap-2'>
                <Button variant='secondary' onClick={handleAddChapters} icon={Plus}>
                  Add Chapter
                </Button>
